...
def raw_points_from_converged_result(refined, camera_matrix, current_k1):
    """
    Muuntaa refine_homography_corrections:in PARHAAN (konvergoituneen)
    kierroksen top-down-pisteet (topdown_src_pts/topdown_dst_pts)
    ...
    Ketju taaksepain on:
      top-down-pikseli
        --(H_before_correction:in KAANTEISHOMOGRAFIA)-->
      oikaistun (jo kerran undistort_points_px:lla vaaristyma-
      korjatun) raakakuvan pikseli
        --(distort_points_px SAMALLA k1:lla jolla se aikoinaan
           oikaistiin)-->
      ALKUPERAISEN raakakuvan (koskaan oikaisematon) pikseli

    Palauttaa (raw_img_pts, phys_pts) - molemmat Nx2 numpy-taulukoita.
    """

    src_pts = np.asarray(refined["topdown_src_pts"], dtype=np.float64)
    dst_pts = np.asarray(refined["topdown_dst_pts"], dtype=np.float64)
    H_before = refined["H_before_correction"]

    H_inv = np.linalg.inv(H_before)

    frame_undistorted_pts = cv2.perspectiveTransform(
        src_pts.reshape(-1, 1, 2).astype(np.float64), H_inv
    ).reshape(-1, 2)

    raw_img_pts = distort_points_px(frame_undistorted_pts, camera_matrix, current_k1)
    phys_pts = output_px_to_physical(dst_pts)
...
def refine_with_hogline_constraints(
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        ratio = measure_far_house_shape_ratio(frame_undistorted, H_new)

        max_angle = max(abs(near_angle), abs(far_angle))

        print(f"  [Hogline-tasoitus {iteration}/{max_iterations}] "
              f"lahempi={near_angle:+.3f} deg, kaukainen={far_angle:+.3f} deg "
              f"(pisteita {len(near_img)}+{len(far_img)}), pyoreys={ratio:.3f}")

        score = (round(max_angle, 1), -ratio)
...
def main():
    ...
    # --------------------------------------------------------
    # OBJEKTIIVIN VAARISTYMAN UUDELLEENARVIOINTI KONVERGOITUNEILLA
    # PISTEILLA (kamera7_03.py, katso refine_lens_distortion_k1:in
    # perustelu)
    #
    # Alkuperainen k1-itsekalibrointi (yllaolevat rivit) kaytti VAIN
    # 22:ta raakakuvasta suoraan (karkealla ellipsisovituksella)
    # saatua korrespondenssipistetta. Nyt kun iteratiivinen homogra-
    # fian korjaus on jo konvergoitunut TARKKOIHIN top-down-tunnis-
    # tuksiin, arvioidaan k1 UUDELLEEN samoilla 17+5 pisteilla mutta
    # paljon puhtaammasta lahteesta. Jos tama paljastaa merkittavan
    # (ja alkuperaisesta selvasti poikkeavan) vaaristyman, koko
    # putki ajetaan KERRAN UUDELLEEN taman uuden k1:n kanssa - ja
    # tulos hyvaksytaan VAIN jos se on OIKEASTI parempi (RMS JA
    # kaukaisen pesan pyoreys) kuin alkuperainen, ei koskaan sokeasti
    # (sama periaate kuin far-korrespondenssikandidaattien valinnassa).
    # --------------------------------------------------------
    ...
    print()
    print("=" * 65)
    print("OBJEKTIIVIN VAARISTYMAN UUDELLEENARVIOINTI (konvergoituneilla pisteilla)")
    print("=" * 65)
    print(f"Nykyinen k1 (alkuperainen arvio)      : {best_k1:+.4f}")
    print(f"RMS ilman korjausta naista pisteista  : {refined_k1_baseline_rms:.2f} px")
    print(f"Uudelleenarvioitu k1                  : {refined_k1:+.4f}")
    ...
    if k1_significant and k1_changed_enough:
        ...
        print(f"-> Kokeillaan uutta k1:aa ({refined_k1:+.4f}) koko putkella uudelleen...")
        ...
        if trial_H is None:
            ...
            try:
                ...
                trial_ratio = measure_far_house_shape_ratio(
                    trial_frame_undistorted, trial_refined["H_final"]
                )
                current_ratio = measure_far_house_shape_ratio(
                    frame_undistorted, refined["H_final"]
                )

                print(f"   Vanha (k1={best_k1:+.4f}): RMS {refined['rms']:.3f} px, "
                      f"kaukaisen pesan pyoreys {current_ratio:.3f}")
                print(f"   Uusi  (k1={refined_k1:+.4f}): RMS {trial_refined['rms']:.3f} px, "
                      f"kaukaisen pesan pyoreys {trial_ratio:.3f}")

                is_better = (
                    trial_ratio > current_ratio + 0.01
                    or (
                        abs(trial_ratio - current_ratio) <= 0.01
                        and trial_refined["rms"] < refined["rms"]
                    )
                )
                ...
                if is_better:
                    ...
                    print(f"Iteratiivinen korjaus (uudella k1:lla) pysahtyi "
                          f"{refined['iterations']} kierroksen jalkeen "
                          f"(RMS {refined['rms']:.3f} px).")
                else:
                    print("   -> Uusi k1 EI parantanut lopputulosta, "
    ...
    # --------------------------------------------------------
    # HOGLINE-TASOITUS (kamera7_04.py)
    #
    # Havaittu: vaikka pesien 22 pistetta sovitetaan hyvin (pieni RMS)
    # ja pesat itse ovat jo lahes taydellisen pyoreita, homografialla
    # on silti riittavasti vapausasteita jattaa hogline USEIDEN
    # ASTEIDEN paahan vaakasuorasta lopullisessa kuvassa - pesien
    # pisteet eivat riita rajoittamaan tata, koska ne ovat paikallisia
    # (kahden pisteen ymparille ryhmittyneita) eivatka suoraan mittaa
    # koko radan leveydelta kulkevan suoran viivan kaltevuutta.
    #
    # detect_hogline_points hakee hoglinen pystysijainnin USEASSA
    # x-segmentissa (robusti, poikkeavat segmentit hylaten - katso sen
    # oma kommentti), ja build_hogline_correspondences muuttaa nama
    # SUORIKSI korrespondenssipisteiksi ("tama pisteen y-koordinaatin
    # PITAA olla tasan hoglinen fyysinen y") jotka lisataan SAMAAN
    # homografian pienimman nelion sovitukseen pesien pisteiden kanssa.
    # refine_with_hogline_constraints toistaa tata (uudet hogline-
    # pisteet tunnistetaan aina hieman korjatusta kuvasta) kunnes
    # tulos ei enaa parane, ja valitsee PARHAAN kierroksen ensisijai-
    # sesti jaljella olevan hogline-kulman mukaan, pyoreyden mukaan
    # vasta toissijaisesti (kayttajan pyynto: hoglinet EHDOTTOMASTI
    # vaakaan, pesat SEN JALKEEN mahdollisimman pyoreiksi).
    #
    # Hyvaksytaan VAIN jos oikeasti parempi - sama periaate kuin
    # kaikkialla muualla tassa tiedostossa.
    # --------------------------------------------------------
    ...
    current_ratio_hog = measure_far_house_shape_ratio(
        frame_undistorted, refined["H_final"]
    )

    print()
    print("=" * 65)
    print("HOGLINE-TASOITUS")
    print("=" * 65)
    print(f"Nykytila ennen tasoitusta: lahempi={current_near_angle:+.3f} deg, "
          f"kaukainen={current_far_angle:+.3f} deg, pyoreys={current_ratio_hog:.3f}")

    hogline_result = refine_with_hogline_constraints(
        frame_undistorted, refined["H_final"], output_w, output_h,
        camera_matrix, best_k1,
        list(raw_img_pts_final), list(phys_pts_final)
    )
    ...
    if hogline_result is not None:
        ...
        print(f"Paras hogline-tasoitettu tulos: lahempi="
              ...
              f"pyoreys={hogline_result['ratio']:.3f} "
              f"({hogline_result['iterations']} kierrosta)")

        # Hyvaksytaan jos hogline-kulma parani (pienempi suurin
        # jaljella oleva kulma) EIKA pyoreys huonontunut merkittavasti
        # (sallitaan pieni, 0.02:n, notkahdus - hoglinen suoristus voi
        # vaatia pienen kompromissin pesan pyoreydessa, mutta ei suurta).
        hogline_improved = new_max_angle < current_max_angle - 0.02
        roundness_acceptable = hogline_result["ratio"] > current_ratio_hog - 0.02

        if hogline_improved and roundness_acceptable:
