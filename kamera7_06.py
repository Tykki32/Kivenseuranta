...
def detect_hogline_points(
    ...
    pisteet joiden etaisyys OLETETUSTA rivista on suuri): VAARIN,
    00008.png:lla TODELLINEN hogline oli itse asiassa ~150 cm PAASSA
    oletetusta (viela hieman epatarkasta) rivista, kun taas HAIRIO oli
    VAIN ~12 cm paassa - siis LAHEMPANA oletettua keskustaa kuin itse
    hogline. "Lahempana keskustaa" ei siis ole luotettava tunnusmerkki.
...
def refine_round(frame, camera_matrix, best_k1, frame_undistorted, refined,
    ...
    # HUOM (kamera7_06.py): hogline-kiertokorjaus (apply_hogline_rotation_
    # correction) EI ole enaa tassa per-kierros-silmukassa - katso
    # apply_final_hogline_rotation:in kommentti siita MIKSI (lyhyesti:
    # kierto siirtaa kaukaista pesaa pivotista sivuun, mika rikkoisi
    # measure_house_quality/expected_house_center_in_crop:in KIINTEAN
    # sijainnin oletuksen KAIKISSA taman jalkeen tulevissa mittauksissa,
    # ei vain taman kierroksen omassa - joten se ajetaan EROTETTUNA,
    # KERRAN, vasta kun taman silmukan (k1+H-yhteisoptimointi) tulos on
    # lopullinen).
...
def main():
    ...
    refined = apply_final_hogline_rotation(
        frame_undistorted, refined, output_w, output_h
    )
